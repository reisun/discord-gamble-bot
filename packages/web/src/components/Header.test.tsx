...
function renderHeader({ isAdmin = false, guildId = null as string | null, token = null as string | null } = {}) {
  vi.mocked(useAuth).mockReturnValue({ token, isAdmin, isVerifying: false, guildId, isTokenExpired: false, isSessionExpired: false, loginUrl: null });
...
describe('Header', () => {
  ...
  it('サーバーアイコンURLが取得できた場合、img タグが表示される', async () => {
    vi.mocked(useAuth).mockReturnValue({ token: null, isAdmin: false, isVerifying: false, guildId: 'guild-123', isTokenExpired: false, isSessionExpired: false, loginUrl: null });
