...
router.get('/:guildId', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const rows = await query<GuildRow>(
      'SELECT guild_id, guild_name, created_at, updated_at FROM guilds WHERE guild_id = $1',
      [req.params.guildId],
    );
    if (rows.length === 0) {
  ...
  try {
    ...
    const rows = await query<GuildRow>(
      `INSERT INTO guilds (guild_id, guild_name)
       ...
       SET guild_name = EXCLUDED.guild_name, updated_at = NOW()
       RETURNING guild_id, guild_name, created_at, updated_at`,
      [req.params.guildId, guildName.trim()],
    );
    res.json({ data: formatGuild(rows[0]) });
  } catch (err) {
    next(err);
