...
**認証**: 有効なトークン必須（`Authorization: Bearer <token>`）。期限切れの場合は `401 TOKEN_EXPIRED` を返す。

`points` および `debt` はキャッシュ値ではなく、クエリパラメータ `eventId` で指定したイベントの履歴テーブルから集計した値を返す。`debt` フィールドは editor ロールのトークンが付与されている場合のみレスポンスに含まれる。
